...
class App:
    ...
    def login(self):
        ...
        if name in ['unknown_person', 'no_persons_found']:
            ...
            sql3 = "SELECT secret_key FROM doctors WHERE id = {}".format(id)
            ...
            self.person_name = result2[0]
            self.person_secret_key = result2[1]
            util.msg_box("Autoryzacja zakończona sukcesem", 'Witaj, {}'.format(self.person_name))
            self.google_authentication()
        
    
    def google_authentication(self):
    	self.main_window.withdraw()
    	self.code_window = tk.Toplevel(self.main_window)
    	self.code_window.geometry("500x500")
    	
    	self.text_label_code = util.get_text_label(self.code_window, 'Podaj kod:')
    	self.text_label_code.place(x=100, y=250)
    	
    	self.entry_text_code = util.get_entry_text(self.code_window)
    	self.entry_text_code.place(x=100, y=350)
    	
    	uri = pyotp.totp.TOTP(self.person_secret_key).provisioning_uri(name=self.person_name, issuer_name='STAHapp')
    	qrcode.make(uri).save("qrcode.jpg")
    	
    	qr = Image.open("qrcode.jpg")
    	qr1 = qr.resize((200,200))
    	obraz_tk = ImageTk.PhotoImage(qr1)
    	label_obraz = tk.Label(self.code_window, image=obraz_tk)
    	label_obraz.place(x=150, y=20)
    	
    	self.accept_button = util.get_button(self.code_window, "Akceptuj", "green", self.verifyCode)
    ...
    def verifyCode(self):
    	code = self.entry_text_code.get(1.0, "end-1c")
    	
    	totp = pyotp.TOTP(self.person_secret_key)
    	status = totp.verify(code)
    	if str(status) == 'False':
    		util.msg_box('Spróbuj ponownie', 'Niepoprawny kod')
    	else:
    		util.msg_box('Sukces', 'Uzyskano dostep do bazy')
